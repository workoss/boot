...
public class ZenRuleEngine {
     ...
     */
    public JsonNode runExpression(String expression, JsonNode json) throws RuleEngineException {
        ...
        EvaluateResult evaluate = evaluate(String.format(expressionTpl, expression).getBytes(StandardCharsets.UTF_8), json, DEFAULT_CONFIG.get());
    ...
    /**
     * 执行方法
     * <p>const handler=(input,{moment,dayjs,Big,env})=>{console.log(input);const momentValid=typeof moment==="function"&&Object.keys(moment).includes('isDayjs');const dayjsValid=typeof dayjs==='function'&&Object.keys(moment).includes('isDayjs');const bigjsValid=typeof Big==='function';return{totalAmount:input.price*input.quantity,momentValid,dayjsValid,bigjsValid};}</p>
     *
    ...
    public JsonNode runFunction(String handlerFunc, JsonNode json) throws RuleEngineException {
        ...
        EvaluateResult evaluate = evaluate(String.format(functionBodyTpl, handlerFunc).getBytes(StandardCharsets.UTF_8), json, zenRuleEngineConfig);
        ...
        if (zenRuleEngineConfig.isTrace()){
            ...
            if (traceResult!=null){
                log.atInfo().log("[RULE_ENGINE] function trace:{}", JsonMapper.toJSONString(traceResult.getTraceData()));
            }
        }
        return evaluate.getResult();
    }
    ...
    public EvaluateResult evaluate(byte[] decision, JsonNode json, ZenRuleEngineConfig customConfig) throws RuleEngineException {
        if (decision == null) {
        ...
        byte[] result = ZenEngineLoader.evaluate(decision, JsonMapper.toJSONBytes(json), customConfig.isTrace(), customConfig.getMaxDepth());
