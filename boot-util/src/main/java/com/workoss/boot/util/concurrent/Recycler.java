...
/**
 * from netty 对象回收池 1、因为具有抽象方法，所以Recycler是抽象类 2、几乎不需要锁
...
public abstract class Recycler<T> {
	...
	/**
	 * 唯一ID生成器 用在两处： 1、当前线程ID 2、WeakOrderQueue的id
	 */
	private static final AtomicInteger ID_GENERATOR = new AtomicInteger(Integer.MIN_VALUE);

	/**
	 * static变量, 生成并获取一个唯一id. 用于pushNow()中的item.recycleId和item.lastRecycleId的设定
	 */
	 ...
	 * 2、Recycler中有且只有两个地方存储DefaultHandle对象（Stack和Link）， 最多可存储MAX_CAPACITY_PER_THREAD +
	 * 最大可共享容量 = 4k + 4k/2 = 6k
	 *
	 * 实际上，在netty中，Recycler提供了两种设置属性的方式 第一种：-Dio.netty.recycler.ratio等jvm启动参数方式
	 * 第二种：Recycler(int maxCapacityPerThread)构造器传入方式
	 */
	private static final int MAX_CAPACITY_PER_THREAD = Math.max(
			Integer.parseInt(System.getProperty("io.netty.recycler.maxCapacityPerThread", String.valueOf(4 * 1024))),
			0);
	...
	/**
	 * 最大可共享的容量因子。 最大可共享的容量 = maxCapacity /
	 * maxSharedCapacityFactor，maxSharedCapacityFactor默认为2
	 */
	private static final int MAX_SHARED_CAPACITY_FACTOR = Math.max(2,
			Integer.valueOf(System.getProperty("io.netty.recycler.maxSharedCapacityFactor", String.valueOf(2))));

	/**
	 * 每个线程可拥有多少个WeakOrderQueue，默认为2*cpu核数 实际上就是当前线程的Map<Stack<?>, WeakOrderQueue>的size最大值
	 */
	private static final int MAX_DELAYE_DQUEUES_PERTHREAD = Math.max(0,
			Integer.parseInt(System.getProperty("io.netty.recycler.maxDelayedQueuesPerThread",
	...
	private final FastThreadLocal<Stack<T>> threadLocal = new FastThreadLocal<Stack<T>>() {
		...
		protected void onRemoved(Stack<T> stack) throws Exception {
			// Let us remove the WeakOrderQueue from the WeakHashMap directly if its safe
			// to remove some overhead
	...
	public final T get() {
		/**
		 * 0、如果maxCapacityPerThread == 0，禁止回收功能 创建一个对象，其Recycler.Handle<User>
		 * handle属性为NOOP_HANDLE，该对象的recycle(Object object)不做任何事情，即不做回收
		 */
	...
	static final class Stack<T> {
		 ...
		 *
		 * from netty: The biggest issue is if we do not use a WeakReference the Thread
		 * may not be able to be collected at all if the user will store a reference to
		 * the DefaultHandle somewhere and never clear this reference (or not clear it in
		 * a timely manner)
		 */
		 ...
		 * 与ratioMask配合，用来决定当前的元素是被回收还是被drop。 例如 ++handleRecycleCount & ratioMask（7），其实相当于
		 * ++handleRecycleCount % 8， 则当 ++handleRecycleCount =
		 * 0/8/16/...时，元素被回收，其余的元素直接被drop
		 */
		private int handleRecycleCount = -1;

		/**
		 * 该值是当线程B回收线程A创建的对象时，线程B会为线程A的Stack对象创建一个WeakOrderQueue对象，
		 * 该WeakOrderQueue指向这里的head，用于后续线程A对对象的查找操作 Q: why volatile? A:
		 * 假设线程A正要读取对象X，此时需要从其他线程的WeakOrderQueue中读取，假设此时线程B正好创建Queue，并向Queue中放入一个对象X；假设恰好次Queue就是线程A的Stack的head
		 * 使用volatile可以立即读取到该queue。
		 *
		 * 对于head的设置，具有同步问题。具体见此处pre的volatile和synchronized void setHead(WeakOrderQueue
		 * queue)
		 */
		private volatile WeakOrderQueue head;

		/**
		 * cursor：当前操作的WeakOrderQueue prev：cursor的前一个WeakOrderQueue
		 */
		private WeakOrderQueue cursor, prev;

		public Stack(Thread threadRef, int maxCapacity, int maxDelayedQueues, int maxSharedCapacityFactor,
				int ratioMask) {
			...
			this.availableSharedCapacity = new AtomicInteger(
					Math.max(maxCapacity / maxSharedCapacityFactor, LINK_CAPACITY));
		...
		DefaultHandle<T> pop() {
			...
			if (size == 0) {
				...
				/**
				 * 由于在transfer(Stack<?>
				 * dst)的过程中，可能会将其他线程的WeakOrderQueue中的DefaultHandle对象传递到当前的Stack,
				 * 所以size发生了变化，需要重新赋值
				 */
				size = this.size;
			}
		...
		private void pushNow(DefaultHandle<T> item) {
			// (item.recycleId | item.lastRecycleId) != 0 等价于 item.recycleId!=0 &&
			// item.lastRecycleId!=0
		...
		/**
		 * 两个drop的时机 1、pushNow：当前线程将数据push到Stack中
		...
		private boolean dropHandle(DefaultHandle<T> item) {
			if (!item.hasBeenRecycled) {
				...
				// 对于pushNow(DefaultHandle<T> item)：该值仅仅用于控制是否执行 (++handleRecycleCount &
				// ratioMask) != 0 这段逻辑，而不会用于阻止重复回收的操作，重复回收的操作由item.recycleId |
				// item.lastRecycledId来阻止
		...
		/**
		 * 假设线程B和线程C同时回收线程A的对象时，有可能会同时newQueue，就可能同时setHead，所以这里需要加锁 以head==null的时候为例， 加锁：
		 * 线程B先执行，则head =
		 * 线程B的queue；之后线程C执行，此时将当前的head也就是线程B的queue作为线程C的queue的next，组成链表，之后设置head为线程C的queue
		 * 不加锁：
	...
	/**
	 * 提供对象的回收功能，由子类进行复写 目前该接口只有两个实现：NOOP_HANDLE和DefaultHandle
	 */
	...
	static final class DefaultHandle<T> implements Handle<T> {
		...
		/**
		 * 标记是否已经被回收： 该值仅仅用于控制是否执行 (++handleRecycleCount & ratioMask) != 0
		 * 这段逻辑，而不会用于阻止重复回收的操作， 重复回收的操作由item.recycleId | item.lastRecycledId来阻止
		 */
		private boolean hasBeenRecycled;

		/**
		 * 只有在pushNow()中会设置值OWN_THREAD_ID 在poll()中置位0
		 */
		private int recycledId;

		/**
		 * pushNow() = OWN_THREAD_ID 在pushLater中的add(DefaultHandle handle)操作中 ==
		 * id（当前的WeakOrderQueue的唯一ID） 在poll()中置位0
		 */
	...
	private static class WeakOrderQueue {
		...
		private static <T> WeakOrderQueue allocate(Stack<T> stack, Thread currentThread) {
			return Head.reserveSpace(stack.availableSharedCapacity, LINK_CAPACITY) ? newQueue(stack, currentThread)
					: null;
		}
		...
		private <T> void add(DefaultHandle<T> item) {
			 ...
			 */
			item.stack = null;
			// we lazy set to ensure that setting stack to null appears before we unnull
			// it in the owning thread;
			// this also means we guarantee visibility of an element in the queue if we
		...
		public <T> boolean transfer(Stack<T> dst) {
			...
			/**
			 * 如果expectedCapacity大于目的地Stack的长度 1、对目的地Stack进行扩容 2、计算Link中最终的可转移的最后一个元素的下标
			 */
		...
		static final class Head {
			...
			/**
			 * 指定读操作的Link节点， eg. Head -> Link1 -> Link2 假设此时的读操作在Link2上进行时，则此处的link ==
			 * Link2，见transfer(Stack dst),
			...
			private static boolean reserveSpace(AtomicInteger availableSharedCapacity, int space) {
				...
				for (;;) {
					...
					// 注意：这里使用的是AtomicInteger，当这里的availableSharedCapacity发生变化时，实际上就是改变的stack.availableSharedCapacity的int
					// value属性的值
