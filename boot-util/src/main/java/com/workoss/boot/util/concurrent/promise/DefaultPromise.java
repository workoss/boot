 ...
 * from netty4.1
 * <p>
 * 一、DefaultPromise状态转换图： A {@link DefaultPromise} is either <em>uncompleted</em> or
 * <em>completed</em>. When an I/O operation begins, a new future object is created. The
 * new future is uncompleted initially - it is neither succeeded, failed, nor cancelled
 * because the I/O operation is not finished yet. If the I/O operation is finished either
 * successfully, with failure, or by cancellation, the future is marked as completed with
 * more specific information, such as the cause of the failure. Please note that even
 * failure and cancellation belong to the completed state. <pre>
 *                                      +---------------------------+
...
public class DefaultPromise<V> extends AbstractFuture<V> implements Promise<V> {
	 ...
	 */
	private static final Throwable CANCELLATION_CAUSE = new CancellationException(
			DefaultPromise.class.getName() + " invoked cancel()");

	/**
	 * Threading - synchronized(this) 事件监听器列表 If {@code empty}, it means either 1) no
	 * listeners were added yet or 2) all listeners were notified. 也就是说
	 * 一个listener通知过一次就会被删除，不会再通知第二次
	 */
	private List<FutureListener<V>> listeners;

	/**
	 * Threading - synchronized(this). We must prevent concurrent notification and FIFO
	...
	public boolean isSuccess() {
		 ...
		 *
		 * 一、是因为
		 * 在下面的代码中会多次调用this.result，当外界的this.result引用发生变化时，由于this.result是被volatile修饰的，如果直接使用this.result将会导致多次获取的result不同，
	...
	public Promise<V> await() throws InterruptedException {
		...
		/**
		 * wait()/notify()机制： 前提：每个对象都有一个锁 + 一个锁等待队列 + 一个条件等待队列。
	...
	/**
	 * 分析并发问题： 1、假设没有notifyingListeners：
	 ...
	 * <p>
	 * 2、加入notifyingListeners： 在线程A执行到第二个synchronized块中的if (this.listeners ==
	 * null)中之前，线程B进入第一个同步块，由于notifyingListeners = true，则直接返回了，
	...
	/**
	 * 设置成功标志 result != null ? result : SUCCESS
	 */
	...
	private boolean setValue0(Object result) {
		/**
		 * 更新result结果，唤醒所有阻塞线程 将result从null置为result 或者
		 * 从UNCANCELLABLE置为result（因为有可能是先将result置为UNCANCELLABLE的）
		 */
	 ...
	 * 关于中断：
	 * <p>
	 * 1、前提：线程有六种状态，{@link Thread#getState()} NEW：A thread that has not yet started is in
	 * this state. RUNNABLE：A thread executing in the Java virtual machine is in this
	 * state. it may be waiting for other resources from the operating system such as
	 * processor. BLOCKED：A thread that is blocked waiting for a monitor lock is in this
	 * state. A thread in the blocked state is waiting for a monitor lock to enter a
	 * synchronized block/method or reenter a synchronized block/method after calling
	 * {@link Object#wait() Object.wait}. WAITING：A thread that is waiting indefinitely
	 * for another thread to perform a particular action is in this state. TIMED_WAITING：A
	 * thread that is waiting for another thread to perform an action for up to a
	 * specified waiting time is in this state. TERMINATED：A thread that has exited is in
	 * this state.
	 * <p>
	 * 2、在不同阶段调用中断Thread.currentThread().interrupt()
	 * NEW/TERMINATED：interrupt()中断没有任何效果，中断位isInterrupted=false RUNNABLE:
	 * interrupt()中断没有效果，中断位isInterrupted=true，在run()方法中自己选择合适的点去处理
