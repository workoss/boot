...
public class InternalThreadLocalMap {
	...
	/**
	 * InternalThreadLocalMap的底层数据结构 其index就是FastThreadLocal的唯一索引index，
	 ...
	 */
	private static final ThreadLocal<InternalThreadLocalMap> SHOW_THREAD_LOCAL_MAP = new ThreadLocal<>();
	...
	private static InternalThreadLocalMap slowGet() {
		InternalThreadLocalMap threadLocalMap = SHOW_THREAD_LOCAL_MAP.get();
		if (threadLocalMap == null) {
			threadLocalMap = new InternalThreadLocalMap();
			SHOW_THREAD_LOCAL_MAP.set(threadLocalMap);
		}
		return threadLocalMap;
	}
	...
	private void expandIndexedVariables(int index, Object value) {
		...
		/**
		 * 计算新数组容量：获取>index的最小的2的n次方的数，例如：1->2 2->4 3->4 4->8 5->8 6->8 7->8 8->16 Returns
		 * a power of two size for the given target capacity. <pre>
		 *
